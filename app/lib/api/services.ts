...
const getCategories = async (lang: string): Promise<ICategory[]> => {
  try {
    const {
      ...
      `/api/categories?locale=${lang}&populate=sub_categories`,
    );
    if (data.length === 0) {
      return notFound();
    }
...
export const getAllProducts = async (
  ...
  { data: IProduct[]; count: number; type?: string } | string | undefined
> => {
  try {
    const {
      data: {
...
export const getProductByUid = async (lang: string, uid: number) => {
  try {
    const {
      ...
      `/api/products?locale=${lang}&filters[uid][$in][0]=${uid}&populate=deep`,
    );
    if (data.length === 0) {
      return notFound();
    }
