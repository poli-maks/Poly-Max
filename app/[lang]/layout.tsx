...
export const generateMetadata = async ({ params: { lang } }: IParams) => {
  return {
    ...
    title: {
      ...
      template: `%s - ${
        lang === "en"
          ? process.env.NEXT_PUBLIC_MAIN_TITLE_EN ||
            "Best metalic production. German quality"
          : process.env.NEXT_PUBLIC_MAIN_TITLE_DE ||
            "Best metalic production. German quality"
      }`,
    },
    description:
      lang === "en"
        ? process.env.NEXT_PUBLIC_MAIN_DESC_EN
        : process.env.NEXT_PUBLIC_MAIN_DESC_DE,
    twitter: {
